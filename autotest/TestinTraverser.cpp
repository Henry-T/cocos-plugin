...
const std::string TestinTraverser::dump() {
    ...
    char platformtStr[20] = {0};
    TargetPlatform platform = TCApplication::sharedApplication()->getTargetPlatform();
    switch(platform){
        case kTargetAndroid:
            sprintf(platformtStr, "%s", "Android");
            break;
        case kTargetIphone:
            sprintf(platformtStr, "%s", "iPhone or iTouch");
            break;
        case kTargetWindows:
            sprintf(platformtStr, "%s", "Windows");
            break;
        case kTargetLinux:
            sprintf(platformtStr, "%s", "Linux");
            break;
        case kTargetMacOS:
            sprintf(platformtStr, "%s", "Mac OS");
            break;
        case kTargetBlackBerry:
            sprintf(platformtStr, "%s", "BlackBerry");
            break;
        case kTargetIpad:
            sprintf(platformtStr, "%s", "iPad");
            break;
    }
    rootElement->SetAttribute("support", platformtStr);
    rootElement->SetAttribute("EngineName", "cocos2d-x");
    rootElement->SetAttribute("version", TESTIN_CC_VERSION);
    memset(scStr, 0, 20);
    sprintf(scStr, "%08X", COCOS2D_VERSION);
    rootElement->SetAttribute("EngineVersionCode", scStr);
...
void TestinTraverser::parseNodeToXML(tinyxml2::XMLDocument *doc, tinyxml2::XMLElement* parent, TCNode* node) {
    ...
    if(baseClass == "TCScene")
    {
        element = doc->NewElement("CureentScene");
        parent->LinkEndChild(element);
        element->SetAttribute("level", "");
        element->SetAttribute("name", "");
        //parseNodeAttr(doc, element, node);
    }else{
        element = doc->NewElement("node");
        parent->LinkEndChild(element);
        //node attributes
        parseNodeAttr(doc, element, node);
    }
